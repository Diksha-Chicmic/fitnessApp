...
const CustomCard: React.FC<CardProps> = ({ title, onPress, percentage, icon, point, }) => {
    ...
    const pointer = {
        style: {
            left: screenWidth / 7.8,
        }
    }
    if (percentage > 10 && percentage < 20) {
        pointer.style.left = screenWidth / 7
    }
    if (percentage > 20 && percentage < 33) {
        pointer.style.left = screenWidth / 4
    }
    if (percentage > 33 && percentage < 66) {
        ...
        pointer.style.left = screenWidth / 1.5
    }
    if (percentage > 66 && percentage < 85) {
        pointer.style.left = screenWidth / 1.2
    }
    else if (percentage > 85) {
        pointer.style.left = screenWidth / 1.08
    }
